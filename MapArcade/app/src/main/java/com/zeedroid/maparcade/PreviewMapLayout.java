...
public class PreviewMapLayout extends View {
...
//    private boolean creatingMap = true;
    ...
    public PreviewMapLayout(Context context) {
        super(context);
    ...
    public PreviewMapLayout (Context context, AttributeSet attrs){
        super(context,attrs);
        this.context = context;
        prefs = PreferenceManager.getDefaultSharedPreferences(context);
        tileSize = Integer.parseInt(prefs.getString("map_tile_size", "512"));
...
//        camera.applyToCanvas(canvas);
    ...
    private void drawPoints(Canvas canvas) {
        ...
        Point point = viewModel.getPoints().get(0);
        ...
        for (int i = 1; i < viewModel.getPoints().size(); i++) {
            point = viewModel.getPoints().get(i);
            pointB = pointA;
            pointA = point;
...
//                route.moveTo((float) pixelX, (float) pixelY);
    ...
    private void drawPointsExtra(Canvas canvas) {
        ...
        for (int j = 0; j < viewModel.getPointsExtra().size(); j++) {
            point = viewModel.getPointsExtra().get(j).getPoint();
            String pointColor = viewModel.getPointsExtra().get(j).getPointColor();
            tc = viewModel.getTileCoordinate(MercatorProjection.tileCoordinateX(point.getLongitude(), viewModel.getZoomLevel(), tileSize),
                    MercatorProjection.tileCoordinateY(point.getLatitude(), viewModel.getZoomLevel(), tileSize));
            if (tc.getTileX() != 0 && tc.getTileY() != 0) {
                tileStartX = tc.getScreenPixelX();
...
//        else red_paintbrush_fill.setTextSize(50f);
        switch (tileSize){
            case 128: black_paintbrush_fill.setTextSize(30f);
                margin = 10;
                gap1   = 30;
                gap2   = 80;
                break;
            case 256: black_paintbrush_fill.setTextSize(60f);
                margin = 20;
                gap1   = 50;
                gap2   = 140;
                break;
            case 512: black_paintbrush_fill.setTextSize(70f);
                margin = 40;
                gap1   = 75;
                gap2   = 240;
                break;
    ...
    private void drawTileLines(Canvas canvas) {
        ...
        if (viewModel.isMapBeingCreated()) drawLineX = startTilePixelX.getTilePixel();
        else                               drawLineX = (int)tc.getScreenPixelX();
        ...
        Log.d("sjd4","screenWidth=" + viewModel.getScreenWidth() + " screenHeight=" + viewModel.getScreenHeight() + " canvasWidth=" + canvas.getWidth() + " canvasHeight=" + canvas.getHeight());
    }
...
//        String currentRoad = viewModel.getPathName(point.getLatitude(), point.getLongitude());
        String currentRoad = MercatorProjection.getPathName(viewModel.getGeocoder(), point.getLatitude(), point.getLongitude());
...
//            if (currentRoad != "" && currentRoad != viewModel.getPathName(point.getLatitude(), point.getLongitude())) {
              if (currentRoad != "" && currentRoad != MercatorProjection.getPathName(viewModel.getGeocoder(), point.getLatitude(), point.getLongitude())) {
                Log.d("ROAD", "currentRoad=" + currentRoad + " NewRoad=" + MercatorProjection.getPathName(viewModel.getGeocoder(), point.getLatitude(), point.getLongitude()));
                if (currentRoad != "") {
                ...
                routeText = new Path();
                routeText.moveTo((float) pixelX, (float) pixelY);
                currentRoad = MercatorProjection.getPathName(viewModel.getGeocoder(), point.getLatitude(), point.getLongitude());
//                currentRoad = viewModel.getPathName(point.getLatitude(), point.getLongitude());
            }
            tc = viewModel.getTileCoordinate(MercatorProjection.tileCoordinateX(point.getLongitude(), viewModel.getZoomLevel(), tileSize),
                    MercatorProjection.tileCoordinateY(point.getLatitude(), viewModel.getZoomLevel(), tileSize));
            if (tc.getTileX() == 0 && tc.getTileY() == 0) {
